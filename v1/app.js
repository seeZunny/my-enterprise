...
function renderSidebar(counts={}){
  ...
  sb.innerHTML='';
  NAV.forEach(g=>{
    const sec=document.createElement('div');sec.className='sb-sec';sec.textContent=g.sec;sb.appendChild(sec);
    g.items.forEach(it=>{
      const n=document.createElement('div');n.className='nav-item';n.dataset.page=it.key;
      const c=counts[it.key];
      n.innerHTML='<span class="nav-icon">'+I[it.icon]+'</span><span>'+it.label+'</span>'
        +(c!=null?'<span class="nav-count">'+c+'</span>':'');
      n.addEventListener('click',()=>{navigate(it.key);closeSidebar();});
      sb.appendChild(n);
    });
  });
...
function toggleSidebar(){document.getElementById('sidebar').classList.toggle('open');document.getElementById('mob-ov').classList.toggle('open');}
...
async function navigate(page){
  ...
  if(crumb){
    crumb.innerHTML='<span style="color:var(--text-3);font-size:13px">'+(NAV.find(g=>g.items.find(i=>i.key===page))?.sec||'')+'</span>'
      +'<span style="color:var(--text-3)">/</span><b>'+esc(PAGE_TITLES[page]||page)+'</b>';
  }
  document.getElementById('tb-actions').innerHTML='';
  const c=document.getElementById('content');
  c.innerHTML='';
  c.style.animation='none';c.offsetWidth;c.style.animation='pageIn .4s cubic-bezier(.4,0,.2,1)';
  const pages={
...
async function pgDashboard(){
  ...
  c.insertAdjacentHTML('beforeend',
    '<div class="ph">'
    +'<div><div class="pt">'+greet+' 👋</div><div class="ps">'+thDate()+' · มีลูกค้าในระบบ '+custs.length+' ราย</div></div>'
    +'<div style="display:flex;gap:8px;flex-wrap:wrap">'
    +'<button class="btn btn-doc btn-quo" onclick="openQuotationForm()">'+I.plus+' ใบเสนอราคา</button>'
    +'<button class="btn btn-doc btn-inv" onclick="openInvoiceForm()">'+I.plus+' ใบกำกับ</button>'
    +'<button class="btn btn-doc btn-rec" onclick="openReceiptForm()">'+I.plus+' ใบเสร็จ</button>'
    +'</div></div>'
  );
  ...
  sg.innerHTML=
    '<div class="stat s-quo"><div class="stat-icon">'+I.quote+'</div><div class="stat-lbl">ใบเสนอราคา</div><div class="stat-val" data-n="'+q.length+'">0</div><div class="stat-meta">รวม <b>'+fmoney(q.reduce((s,i)=>s+(i.total||0),0))+'</b> บาท</div></div>'
    +'<div class="stat s-inv"><div class="stat-icon">'+I.invoice+'</div><div class="stat-lbl">ใบกำกับภาษี</div><div class="stat-val" data-n="'+inv.length+'">0</div><div class="stat-meta">ยอดขายรวม <b>฿'+fmoney(tInv)+'</b></div></div>'
    +'<div class="stat s-rec"><div class="stat-icon">'+I.receipt+'</div><div class="stat-lbl">ใบเสร็จรับเงิน</div><div class="stat-val" data-n="'+rec.length+'">0</div><div class="stat-meta">รับเงินแล้ว <b>฿'+fmoney(tRec)+'</b></div></div>'
    +'<div class="stat s-bil"><div class="stat-icon">'+I.billing+'</div><div class="stat-lbl">ใบวางบิล</div><div class="stat-val" data-n="'+bil.length+'">0</div><div class="stat-meta">ค้างชำระ <b style="color:var(--warning)">'+pending.length+'</b> ใบ · ฿'+fmoney(tPend)+'</div></div>';
  c.appendChild(sg);
  // animate counters
  setTimeout(()=>{
    sg.querySelectorAll('.stat-val').forEach(el=>{
      const target=parseInt(el.dataset.n)||0;
      animateNumber(el,0,target,800,v=>Math.round(v).toLocaleString('th-TH'));
    });
  },80);
  ...
  recCard.innerHTML='<div class="card-header"><div><div class="card-title">เอกสารล่าสุด</div><div class="card-sub">8 รายการล่าสุด</div></div><button class="btn btn-soft btn-sm" onclick="navigate(\'docs\')">ดูทั้งหมด '+I.arrowRight+'</button></div>';
  ...
  if(!recent.length){recBody.innerHTML='<div class="empty"><div class="empty-i">'+I.docs+'</div><div class="empty-t">ยังไม่มีเอกสาร</div><div class="empty-s">สร้างเอกสารแรกของคุณได้เลย</div></div>';}
    ...
    row.style.cssText='padding:12px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:12px;cursor:pointer;transition:background var(--t-fast);animation:fadeIn .3s ease '+(i*30)+'ms both';
    row.onmouseover=()=>row.style.background='var(--surface-2)';row.onmouseout=()=>row.style.background='';
    row.onclick=()=>viewDocModal(d._s,d.id);
    row.innerHTML='<div style="min-width:0;flex:1"><div style="font-weight:600;font-size:13.5px;color:'+d._col+';font-variant-numeric:tabular-nums">'+esc(d.docNumber||'')+'</div><div style="font-size:12px;color:var(--text-3);margin-top:2px">'+esc(d.customerName||'-')+' · '+thDateShort(d.docDate)+'</div></div>'
      +'<div style="text-align:right;flex-shrink:0"><span class="badge '+d._cls+'">'+d._lbl+'</span><div style="font-size:13px;font-weight:600;margin-top:4px" class="tnum">฿'+fmoney(d.total)+'</div></div>';
  ...
  pendCard.innerHTML='<div class="card-header"><div><div class="card-title">ใบวางบิลค้างชำระ</div><div class="card-sub">รอรับชำระเงิน</div></div></div>';
  const pendBody=document.createElement('div');
  if(!pending.length){pendBody.innerHTML='<div class="empty"><div class="empty-i" style="background:var(--rec-soft);color:var(--rec)">'+I.check+'</div><div class="empty-t">ไม่มีค้างชำระ</div><div class="empty-s">เยี่ยม! ทุกอย่างเรียบร้อย</div></div>';}
  else pending.slice(0,6).forEach((b,i)=>{
    const row=document.createElement('div');
    row.style.cssText='padding:12px 20px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;gap:12px;animation:fadeIn .3s ease '+(i*30)+'ms both';
    row.innerHTML='<div style="min-width:0;flex:1"><div style="font-weight:600;color:var(--bil);font-size:13px">'+esc(b.docNumber)+'</div><div style="font-size:12px;color:var(--text-3)">'+esc(b.customerName)+'</div></div>'
      +'<div style="font-weight:700;color:var(--bil)" class="tnum">฿'+fmoney(b.total)+'</div>';
  ...
  tip.style.cssText='margin-top:20px;background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);color:#fff;border-radius:var(--r-lg);padding:24px 28px;display:flex;align-items:center;gap:24px;position:relative;overflow:hidden';
  tip.innerHTML='<div style="position:absolute;right:-40px;top:-40px;width:200px;height:200px;background:radial-gradient(circle,rgba(20,184,166,.18) 0%,transparent 70%);pointer-events:none"></div>'
    +'<div style="width:56px;height:56px;border-radius:14px;background:linear-gradient(135deg,#14b8a6,#0d9488);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;box-shadow:0 8px 24px rgba(20,184,166,.4)">'+I.sparkles+'</div>'
    +'<div style="flex:1"><div style="font-size:15px;font-weight:600;letter-spacing:-.01em;margin-bottom:4px">เคล็ดลับ — ใช้ Command Palette</div><div style="font-size:13px;color:#94a3b8">กด <kbd style="background:rgba(255,255,255,.08);padding:2px 8px;border-radius:5px;font-family:var(--font-mono);font-size:11px">⌘K</kbd> หรือ <kbd style="background:rgba(255,255,255,.08);padding:2px 8px;border-radius:5px;font-family:var(--font-mono);font-size:11px">Ctrl+K</kbd> เพื่อเปิดเมนูค้นหา สร้างเอกสาร หรือไปยังหน้าใดก็ได้แบบรวดเร็ว</div></div>'
    +'<button class="btn btn-accent" onclick="openCmdK()">เปิด Command Palette '+I.arrowRight+'</button>';
  c.appendChild(tip);
}
...
async function renderDocList(cfg){
  ...
  const tb=document.getElementById('tb-actions');tb.innerHTML='';
  const ab=document.createElement('button');ab.className='btn btn-doc btn-'+icon;
  ab.innerHTML=I.plus+' '+(addLabel||('สร้าง'+title));
  ab.addEventListener('click',addFn);tb.appendChild(ab);

  const c=document.getElementById('content');c.innerHTML='';
  c.insertAdjacentHTML('beforeend','<div class="ph"><div><div class="pt">'+title+'</div><div class="ps">ทั้งหมด '+docs.length+' รายการ</div></div></div>');
...
async function pgSearch(){
  const c=document.getElementById('content');c.innerHTML='';
  c.insertAdjacentHTML('beforeend',
    '<div class="ph"><div><div class="pt">ค้นหาเอกสาร</div><div class="ps">ค้นจากเลขที่เอกสาร หรือชื่อลูกค้า</div></div></div>'
...
async function pgCustomers(){
  ...
  const tb=document.getElementById('tb-actions');tb.innerHTML='';
  const ab=document.createElement('button');ab.className='btn btn-accent';ab.innerHTML=I.plus+' เพิ่มลูกค้า / คู่ค้า';
  ab.addEventListener('click',()=>openCustomerForm());tb.appendChild(ab);
  const c=document.getElementById('content');c.innerHTML='';
  c.insertAdjacentHTML('beforeend','<div class="ph"><div><div class="pt">ลูกค้า / คู่ค้า</div><div class="ps">ทั้งหมด '+custs.length+' ราย · ระบบจะจดจำที่อยู่อัตโนมัติเมื่อออกเอกสาร</div></div></div>');
...
async function pgInventory(){
  ...
  ab.addEventListener('click',()=>openInventoryForm());tb.appendChild(ab);
  const c=document.getElementById('content');c.innerHTML='';
  c.insertAdjacentHTML('beforeend','<div class="ph"><div><div class="pt">คลังสินค้า</div><div class="ps">'+items.length+' รายการ · ใช้ใน autocomplete เมื่อสร้างเอกสาร</div></div></div>');
  const card=document.createElement('div');card.className='card';
  const head=document.createElement('div');head.className='card-header';
  const sbar=document.createElement('div');sbar.className='sbar';sbar.style.width='320px';
  sbar.innerHTML=I.search;
